...
from model.params import default_setup, G
...
from model.sim import run_matrix, print_table, METRIC_COLS


def _plot_series(ax, results, xkey, ykey, transform=None, ls="-"):
    """One line per controller config, fixed colors, combined drawn on top.

    Config identity is carried by COLOR; a second series on the same axes (the
    other axle, say) is distinguished by linestyle, never by hue — see style.py."""
    for name, res in results.items():
        log = res["log"]
        y = log[ykey] if transform is None else transform(log)
        ax.plot(log[xkey], y, color=color_for(name), lw=config_lw(name),
                ls=ls, zorder=config_z(name),
                label=name if ls == "-" else None)


def _footer(fig, maneuver, tag):
    fig.text(0.01, 0.005,
             f"{tag} · {maneuver.name}: {maneuver.description} · all "
...
def plot_wheels(plt, maneuver, results, outdir, tag="", k_spin=None,
    ...
    ax = axs[0, 1]
    # worst of the four corners, not kRL: the old panel plotted the rear-left
    # wheel and called it "inner", which is only true in a LEFT-hand corner —
    # in a right-hander (--track-right, or the slalom's second half) it was
    # quietly showing the OUTER wheel. Max over four is right either way, and
    # right for a car whose front wheels are now driven too.
    _plot_series(ax, results, "t", "kRL", transform=lambda l: np.max(
        np.abs(np.stack([l["kFL"], l["kFR"], l["kRL"], l["kRR"]])), axis=0))
    if k_spin:
    ...
    ax.set_title("Worst-corner slip ratio — the wheelspin watch\n"
                 "(above the red line more spin makes LESS force)",
                 fontsize=10)
    ax.set_xlabel("time  t  [s]")
    ax.set_ylabel("max |κ| over the four wheels  [–]")

    ax = axs[1, 0]
    _plot_series(ax, results, "t", "r",
                 transform=lambda l: l["T_RR"] - l["T_RL"])
    _plot_series(ax, results, "t", "r",
                 transform=lambda l: l["T_FR"] - l["T_FL"], ls="--")
    ax.set_title("Commanded torque split per axle — the actuator itself\n"
                 "(solid = rear, dashed = front; positive = more torque RIGHT "
                 "→ yaws LEFT)", fontsize=10)
    ax.set_xlabel("time  t  [s]")
    ax.set_ylabel("torque split  ΔT = T_R − T_L  [N·m]")
    ...
    n_drv = vp_ref.driven_wheels if vp_ref is not None else 0
    if p_ceiling_kw and p_ceiling_kw < 0.99 * 80.0:
        ax.axhline(p_ceiling_kw, ls="--", color=REF_COLOR, lw=1.4, zorder=1,
                   label=f"{n_drv:.0f}-motor ceiling = {p_ceiling_kw:.0f} kW "
                         f"@ {cfg.drivetrain.pack_voltage:g} V")
    ax.set_title("Total mechanical drive power\n"
                 + ("(motor ceiling and rules cap now coincide — no headroom)"
                    if p_ceiling_kw and p_ceiling_kw >= 0.99 * 80.0
                    else "(the motor ceiling is below the rules cap)"),
                 fontsize=10)
    ax.set_xlabel("time  t  [s]")
...
def build_maneuvers(args, vp, interactive):
    ...
    # corner-exit throttle is stored as N·m in maneuvers.py; the user-facing
    # knob is a % of what the TIRES can take, not of what the motors can
    # deliver — same reference maneuvers.corner_exit() uses, so the flag's
    # default reproduces the maneuver's default exactly and neither moves when
    # the drivetrain layout changes.
    peak = cfg.tires.mu0_long * vp.m_total * G * vp.r_wheel
    cd_throttle_pct = 35.0
